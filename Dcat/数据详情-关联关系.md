...
> 为了能够正常使用这个面板右上角的工具，必须用 `setResource` 方法设置用户资源的 url 访问路径。
...
注意：为了能够正常使用这个数据表格的功能，必须用 `setResource()` 方法设置 `comments` 资源的 url 访问路径

## 多对多

多对多会以[数据表格](https://learnku.com/docs/dcat-admin/2.x/relationship/model-grid.md)的方式呈现，下面是简单的例子
...
$show->relation('permissions', function ($model) {
    ...
    // 设置路由
    $grid->setResource('setting/permissions');
